...
export default function AuditClient({ lang }: { lang: "en" | "zh" }) {
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-500">
            ...
            <div className="bg-[#1a1a1a]/40 border border-white/5 rounded-[40px] overflow-hidden backdrop-blur-md shadow-2xl">
                        ...
                        <tbody className="divide-y divide-white/[0.02]">
                            {filteredLogs.map((log, i) => (
                                <tr key={i} className="text-sm group hover:bg-white/[0.02] transition-colors">
                                    <td className="px-8 py-6 text-zinc-500 font-mono text-[10px]">{new Date(log.created_at).toLocaleString()}</td>
                                    <td className="px-8 py-6">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className="font-black text-white uppercase tracking-tight text-xs">{log.action}</span>
                                        </div>
                                    </td>
                                    <td className="px-8 py-6 text-zinc-400 font-bold text-xs">{log.user_email}</td>
                                    <td className="px-8 py-6">
                                        <div className="text-[10px] font-black uppercase tracking-widest text-zinc-500">{log.admin_username}</div>
                                    </td>
                                    <td className="px-8 py-6 text-right">
                                        <div className="text-[10px] text-zinc-600 font-bold uppercase tracking-tighter truncate max-w-[200px] ml-auto">
                            ...
                            {filteredLogs.length === 0 && (
                                <tr><td colSpan={5} className="p-20 text-center text-zinc-700 font-black uppercase tracking-widest">{t.noLogs}</td></tr>
                            )}
                        </tbody>
                    </table>
                </div>
