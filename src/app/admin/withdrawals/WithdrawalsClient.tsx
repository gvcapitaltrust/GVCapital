            ...
            <div className="bg-white backdrop-blur-md rounded-3xl border border-slate-200 overflow-hidden shadow-xl">
                <div className="overflow-x-auto">
                                ...
                                <tr key={idx} className="text-sm group hover:bg-slate-50 transition-all font-medium">
                                    <td className="px-4 py-1.5 min-w-[140px]">
                                        <div className="flex flex-col">
                                            <span className="font-black text-slate-900 uppercase tracking-tight whitespace-nowrap">{tx.profiles?.full_name}</span>
                                    ...
                                    <td className="px-4 py-1.5">
                                        {tx.metadata?.penalty_applied || tx.metadata?.penalty_amount ? (
                                            <div className="flex flex-col">
                                                <span className="font-bold text-red-600 tabular-nums text-xs whitespace-nowrap">$ {(Number(tx.metadata?.original_usd_penalty || (Number(tx.metadata?.finalized_penalty || tx.metadata?.penalty_amount || 0) / forexRate))).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                                                <span className="text-[8px] text-slate-400 font-black uppercase tracking-widest leading-none">40% Early</span>
                                            </div>
                                        ) : (
                                            <span className="text-slate-400 font-bold uppercase text-[9px] leading-none">-</span>
                                        )}
                                    </td>
                                    <td className="px-4 py-1.5">
                                        <div className="flex flex-col leading-tight">
                                            <span className="text-[9px] font-black text-slate-900 truncate uppercase tracking-tighter">
                                    ...
                                    <td className="px-4 py-1.5">
                                        <div className="flex flex-col">
                                            <span className="font-black text-emerald-600 tabular-nums text-xs whitespace-nowrap">$ {(Number(tx.metadata?.original_usd_payout || (Number(tx.metadata?.finalized_payout || tx.metadata?.expected_payout || Math.abs(Number(tx.amount))) / forexRate))).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                                        </div>
                                    </td>
                                    <td className="px-4 py-1.5 text-slate-400 font-mono text-[10px] whitespace-nowrap">{new Date(tx.created_at).toLocaleDateString()}</td>
                                    ...
                                    <td className="px-4 py-1.5 text-right">
                                        <div className="flex items-center justify-end gap-3">
                                            {tx.status === 'Pending' && (
                                                <>
                                                    <button onClick={() => handleRejectWithdrawal(tx)} className="text-red-500 hover:bg-red-50 px-4 py-2 rounded-xl text-[9px] font-black uppercase transition-all">{t.reject}</button>
                                                    <button onClick={() => handleApproveWithdrawal(tx)} className="bg-emerald-600 text-white px-4 py-2 rounded-xl text-[9px] font-black uppercase shadow-lg shadow-emerald-500/10 hover:-translate-y-0.5 transition-all">{t.approve}</button>
                                                </>
                                            )}
                                            ...
                                            {tx.status === 'Completed' && <span className="text-[10px] text-slate-400 font-black uppercase italic">Processed</span>}
                                            {tx.status === 'Approved' && <span className="text-[10px] text-slate-400 font-black uppercase italic">Legacy Success</span>}
                                            {tx.status === 'Rejected' && <span className="text-[10px] text-slate-400 font-black uppercase italic">Declined</span>}
                                        </div>
                                    </td>
                                </tr>
                            ))}
                    ...
                    {filteredWithdrawals.length === 0 && (
                        <div className="p-20 text-center text-slate-500 font-black uppercase tracking-[0.2em]">{t.noWithdrawals}</div>
                    )}
                </div>
            </div>
        </div>
