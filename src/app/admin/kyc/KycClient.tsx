                ...
                <div className="overflow-x-auto overflow-y-auto max-h-[650px] scrollbar-thin scrollbar-thumb-gray-200">
                    <table className="w-full text-left border-collapse hidden md:table">
                        <thead className="bg-slate-50/50 border-b border-slate-100 sticky top-0 z-10 backdrop-blur-md">
                            <tr className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">
                                <th className="px-4 py-4 pl-8">Client Name</th>
                                <th className="px-4 py-4">Identity Type</th>
                                <th className="px-4 py-4">Verification Status</th>
                                <th className="px-4 py-4 pr-8 text-right">Review Account</th>
                            </tr>
                        </thead>
                                    ...
                                    <td className="px-4 py-4 pl-8">
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div className="flex flex-col overflow-hidden">
                                                <span className="font-black text-slate-900 uppercase tracking-tight text-[13px] group-hover:text-gv-gold transition-colors truncate max-w-[150px]">{user.full_name}</span>
                    ...
                    <div className="md:hidden divide-y divide-slate-50">
                        {filteredKyc.map((user: any, idx: number) => {
                            ...
                            return (
                                <div key={user.id || idx} className="p-3 space-y-3 hover:bg-slate-50 transition-all flex flex-col" onClick={() => openDetails(user)}>
                                    <div className="flex justify-between items-start">
                                        <div className="flex items-center gap-3">
                                            <TierMedal tierId={tierName} size="xs" />
                                            <div className="flex flex-col">
                                                <span className="font-black text-slate-900 uppercase tracking-tight text-[10px] truncate max-w-[150px]">{user.full_name}</span>
