        ...
        <div className="space-y-8 animate-in fade-in duration-500">
                                    ...
                                    <tr key={idx} className="text-sm group hover:bg-gray-50 transition-all">
                                        <td className="px-8 py-6">
                                            <div className="flex items-center gap-4">
                                                <div className="h-10 w-10 rounded-xl bg-gradient-to-br from-gv-gold/20 to-slate-200 border border-slate-200 flex items-center justify-center font-black text-gv-gold text-xs shadow-inner">
                                                    {(user.full_name || user.username || "?")[0].toUpperCase()}
                                                </div>
                                                <div className="flex flex-col">
                                                    <span className="font-extrabold text-slate-900 uppercase tracking-tight text-xs">{user.full_name || user.username}</span>
                                                    <span className="text-[9px] text-slate-500 font-bold uppercase tracking-widest">{user.email}</span>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="px-8 py-6">
                                            <div className="flex flex-col gap-0.5">
                                                <div className="flex items-baseline gap-1.5">
                                                    <span className="text-[14px] font-black text-slate-900 tabular-nums">$</span>
                                                    <span className="text-[16px] font-black text-slate-900 tabular-nums">{(user.total_assets_usd || 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="px-8 py-6">
                                            ...
                                            {(() => {
                                                ...
                                                return (
                                                    <div className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full border shadow-sm transition-all ${
                                                        ...
                                                        <div className={`h-1.5 w-1.5 rounded-full ${isNoTier ? 'bg-slate-300' : 'bg-gv-gold shadow-[0_0_8px_rgba(212,175,55,0.5)]'}`}></div>
            ...
            {isDetailModalOpen && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-12 animate-in fade-in duration-500">
                    <div className="absolute inset-0 bg-slate-900/60 backdrop-blur-2xl" onClick={() => setIsDetailModalOpen(false)}></div>
                    <div className="relative bg-white border border-slate-200 rounded-[40px] w-full max-w-7xl h-full flex flex-col overflow-hidden shadow-2xl">
                        <div className="p-8 border-b border-slate-100 flex items-center justify-between bg-slate-50">
                            <div className="flex items-center gap-6">
                                ...
                                <div>
                                    <h3 className="text-2xl font-black uppercase tracking-tighter text-slate-900">{selectedUser?.full_name}</h3>
                                    <div className="flex items-center gap-3">
                                        ...
                                        <span className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">@{selectedUser?.username}</span>
                                    </div>
                                </div>
                            </div>
                            <div className="flex items-center gap-4">
                                ...
                                >
                                    {isRoleChanging ? "..." : (selectedUser?.role?.toLowerCase() === 'admin' ? "Remove Admin" : "Promote to Admin")}
                                </button>
                                <button 
                                    onClick={executeToggleStatus} 
                                    disabled={isProcessingAction}
                                    className="bg-orange-50 text-orange-600 hover:bg-orange-600 hover:text-white text-[9px] font-black uppercase tracking-widest px-6 py-3 rounded-2xl transition-all border border-orange-200 disabled:opacity-50"
                                >
                                ...
                                <button onClick={() => handleResetUserPassword(selectedUser.email)} className="bg-white hover:bg-slate-50 text-[9px] font-black uppercase tracking-widest px-6 py-3 rounded-2xl transition-all border border-slate-200 text-slate-900">{t.resetPassword}</button>
                                <button onClick={() => setIsDetailModalOpen(false)} className="h-12 w-12 bg-slate-100 rounded-full flex items-center justify-center hover:bg-slate-200 transition-all text-slate-500">
                        ...
                        <div className="flex-1 overflow-y-auto p-8 custom-scrollbar bg-slate-50/50">
                            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                                <div className="space-y-8">
                                    <div className="bg-white rounded-[32px] p-8 border border-slate-200 space-y-8 shadow-sm">
                                        <h4 className="text-[10px] font-black uppercase tracking-widest text-gv-gold">{t.adjustHeader}</h4>
                                        <div className="space-y-6">
                                            <div className="space-y-2">
                                                <label className="text-[9px] font-black uppercase text-slate-400 px-1">{t.adjustAmount}</label>
                                                <input 
                                                    type="number" 
                                                    value={adjustmentAmount}
                                                    onChange={(e) => setAdjustmentAmount(e.target.value)}
                                                    className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-4 text-slate-900 font-black text-xl focus:outline-none focus:border-gv-gold transition-all"
                                                    placeholder="0.00"
                                                />
                                            </div>
                                            <div className="space-y-2">
                                                <label className="text-[9px] font-black uppercase text-slate-400 px-1">{t.adjustType}</label>
                                                <div className="grid grid-cols-2 gap-2">
                                                    <button 
                                                        onClick={() => setAdjustmentType("balance")}
                                                        className={`py-3 rounded-xl text-[9px] font-black uppercase tracking-widest border transition-all ${adjustmentType === 'balance' ? 'bg-gv-gold text-black border-gv-gold' : 'bg-white text-slate-400 border-slate-200'}`}
                                                    >
                                                        {t.typeBalance}
                                                    </button>
                                                    <button 
                                                        onClick={() => setAdjustmentType("profit")}
                                                        className={`py-3 rounded-xl text-[9px] font-black uppercase tracking-widest border transition-all ${adjustmentType === 'profit' ? 'bg-gv-gold text-black border-gv-gold' : 'bg-white text-slate-400 border-slate-200'}`}
                                                    >
                                                        {t.typeProfit}
                                                    </button>
                                                </div>
                                            ...
                                            <div className="space-y-2">
                                                <label className="text-[9px] font-black uppercase text-slate-400 px-1">{t.adjustReason}</label>
                                                <textarea 
                                                    value={adjustmentReason}
                                                    onChange={(e) => setAdjustmentReason(e.target.value)}
                                                    className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-4 text-slate-900 text-xs font-medium focus:outline-none focus:border-gv-gold transition-all min-h-[100px]"
                                                    placeholder="..."
                                                />
                                            </div>
                                            <button 
                                ...
                                <div className="space-y-8 lg:col-span-2">
                                    <div className="bg-white rounded-[32px] p-8 border border-slate-200 space-y-8 shadow-sm">
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                            <div className="space-y-2">
                                                <label className="text-[9px] font-black uppercase text-slate-400 px-1">{t.portfolioPlatform}</label>
                                                <input 
                                                    value={portfolioData.platform}
                                                    onChange={(e) => setPortfolioData(prev => ({ ...prev, platform: e.target.value }))}
                                                    className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-4 text-slate-900 text-xs font-bold focus:outline-none"
                                                />
                                            </div>
                                            <div className="space-y-2">
                                                <label className="text-[9px] font-black uppercase text-slate-400 px-1">{t.portfolioAccount}</label>
                                                <input 
                                            ...
                                            <div className="space-y-2">
                                                <label className="text-[9px] font-black uppercase text-slate-400 px-1">{t.portfolioPassword}</label>
                                                <input 
                                                    type="password"
                                            ...
                                            <div className="space-y-2">
                                                <label className="text-[9px] font-black uppercase text-slate-400 px-1">Internal Reference</label>
                                                <input 
                                                    className="w-full bg-slate-100 border border-slate-200 rounded-2xl p-4 text-slate-400 text-xs font-bold"
                                                    readOnly
                                                    value={selectedUser?.id}
                                                />
                                            </div>
                                            <div className="md:col-span-2 space-y-2">
                                                <label className="text-[9px] font-black uppercase text-slate-400 px-1">{t.portfolioRemarks}</label>
                                                <textarea 
                                                    value={portfolioData.remarks}
                                                    onChange={(e) => setPortfolioData(prev => ({ ...prev, remarks: e.target.value }))}
                                                    className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-4 text-slate-900 text-xs font-medium focus:outline-none min-h-[100px]"
                                                />
                                            </div>
                                        </div>
                                        <button 
