...
export default function DepositAgreementModal({
    ...
    const canSubmit = agreed && nameMatches && !submitting;
    ...
    return (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 sm:p-6 bg-gray-900/60 backdrop-blur-md animate-in fade-in duration-300">
            <div className="bg-white border border-gray-200 rounded-[2rem] max-w-4xl w-full max-h-[95vh] overflow-hidden shadow-2xl relative flex flex-col">
                {onClose && (
                    <button
                        onClick={onClose}
                ...
                <div className="px-6 sm:px-10 pt-6 pb-4 border-b border-gray-200 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                    <div className="flex items-center gap-3 min-w-0 pr-10">
                        <div className="h-9 w-9 rounded-xl bg-gv-gold/10 flex items-center justify-center shrink-0">
                            <ShieldCheck className="h-5 w-5 text-gv-gold" />
                        </div>
                        <div className="min-w-0">
                            <h2 className="text-base sm:text-lg font-black text-gray-900 uppercase tracking-tight truncate">
                    ...
                    <button
                        ...
                        disabled={viewingPdf}
                        className="inline-flex items-center gap-2 px-3 py-2 rounded-lg bg-white border border-gray-200 hover:bg-gray-100 transition-all text-[10px] sm:text-[11px] font-black uppercase tracking-widest text-gray-700 disabled:opacity-50 shrink-0 self-start sm:self-auto"
                    >
                ...
                {/* Single scroll area: agreement body + sign block at the end */}
                <div
                    ref={bodyRef}
                    className="flex-1 overflow-y-auto"
                >
                    {/* Agreement content */}
                    <div className="px-6 sm:px-10 py-6 text-gray-700 leading-relaxed">
                    ...
                    {/* Sign block — inline at the bottom of the scroll */}
                    <div className="border-t border-gray-200 bg-gray-50 px-6 sm:px-10 py-6 space-y-4">
                        <h3 className="text-sm font-black uppercase tracking-wider text-gray-900">
                            {lang === "en" ? "Sign & Confirm" : "签署确认"}
                        </h3>

                        <label className="flex items-start gap-3 text-[12px] sm:text-[13px] font-medium text-gray-900 cursor-pointer">
                            <input
                                type="checkbox"
                                checked={agreed}
                                onChange={(e) => setAgreed(e.target.checked)}
                                className="mt-0.5 h-4 w-4 accent-gv-gold"
                            />
