        ...
        <>
            ...
            <aside className={`border-r border-slate-200 flex flex-col justify-between hidden lg:flex bg-slate-50 transition-all duration-500 ease-in-out relative group/sidebar z-50 overflow-y-auto ${isCollapsed ? "w-[84px] p-4" : "w-64 p-6"}`}>
                <button 
                    ...
                    className="absolute -right-3 top-24 z-10 h-6 w-6 bg-white border border-slate-200 rounded-full flex items-center justify-center text-slate-500 hover:bg-gv-gold hover:text-white transition-all shadow-lg opacity-0 group-hover/sidebar:opacity-100"
                >
                        ...
                        {menuItems.map(item => {
                            ...
                            return (
                                <Link
                                    ...
                                    } ${isActive ? "bg-gv-gold/10 text-gv-gold border border-gv-gold/20" : "text-slate-500 hover:text-slate-900 hover:bg-slate-100"}`}
                ...
                <div className="space-y-4 pt-4 border-t border-slate-200 shrink-0">
                    <div className={`flex items-center px-4 pb-2 transition-all duration-500 ${isCollapsed ? "justify-center" : "justify-between"}`}>
                        <div className="flex items-center gap-4">
                            ...
                            {!isCollapsed && <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">{t.maintenance}</span>}
                        </div>
                        <button onClick={onToggleMaintenance} className={`h-5 rounded-full relative transition-all ${isCollapsed ? "w-8" : "w-10"} ${maintenanceMode ? "bg-red-500" : "bg-slate-200"}`}>
                    ...
                    <button onClick={handleLogout} className={`w-full text-slate-400 hover:text-red-500 transition-colors flex items-center ${isCollapsed ? "justify-center p-3" : "gap-4 px-4 py-2.5"} rounded-2xl hover:bg-red-50`}>
            ...
            />
            <aside
                className={`fixed inset-y-0 left-0 z-[60] w-80 bg-slate-50 border-r border-slate-200 p-8 flex flex-col justify-between transition-transform duration-500 ease-out lg:hidden ${
            ...
            >
                <div className="space-y-12">
                    <div className="flex items-center justify-between">
                        ...
                        <button
                            onClick={onCloseMobileMenu}
                            className="h-10 w-10 flex items-center justify-center rounded-full border border-slate-200 text-slate-400 hover:text-slate-900 transition-colors"
                        >
                    ...
                    <div className="space-y-8">
                        <div>
                            <p className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-500 mb-4 px-2">Menu</p>
                                ...
                                {menuItems.map((item) => (
                                    <button
                                        ...
                                        className={`w-full flex items-center gap-4 px-4 py-4 rounded-2xl text-[11px] font-black uppercase tracking-widest transition-all ${
                                            pathname === item.path ? "bg-gv-gold/10 text-gv-gold border border-gv-gold/20" : "text-slate-400 hover:text-slate-900 hover:bg-white"
                                        }`}
                                    >
                                        {item.icon}
                                        {item.label}
                ...
                <div className="space-y-6">
                    <div className="p-6 bg-white rounded-[32px] border border-slate-200 space-y-4">
                        <div className="flex items-center justify-between text-[11px] font-black uppercase tracking-widest text-slate-400 px-2 pb-2 border-b border-slate-100">
                            <span>{t.maintenance}</span>
                            <button onClick={onToggleMaintenance} className={`h-5 rounded-full relative transition-all w-10 ${maintenanceMode ? "bg-red-500" : "bg-slate-200"}`}>
                    ...
                    return (
                        <Link
                            ...
                            className={`group relative flex flex-col items-center justify-center w-16 h-16 transition-all duration-300 ${
                                isActive ? "scale-110" : "hover:bg-slate-50 rounded-2xl"
                            }`}
                        >
                            {isActive && (
                                <div className="absolute inset-0 bg-gv-gold/10 rounded-2xl border border-gv-gold/20 shadow-inner"></div>
                            )}
                            <span className={`relative z-10 transition-colors duration-300 ${isActive ? "text-gv-gold" : "text-slate-400 group-hover:text-slate-900"}`}>
                                {item.icon}
                            </span>
                            <span className={`absolute -bottom-1 text-[8px] font-black uppercase tracking-widest transition-all duration-300 ${isActive ? "text-gv-gold opacity-100 translate-y-3" : "text-slate-400 opacity-0 translate-y-0"}`}>
                                {item.label}
                            </span>
                        </Link>
                    );
